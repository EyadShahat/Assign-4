...
saveName.addEventListener('click', () => {
  ...
  applyName(v);
  nameFeedback.textContent = v ? 'Saved!' : 'Name cleared.';
...
function showRepoSkeletons() {
  reposList.innerHTML = '';
  for (let i = 0; i < 4; i++) {
    const div = document.createElement('div');
    div.className = 'repo-card repo-skeleton skeleton';
...
function renderRepos() {
  ...
  arr.forEach(r => {
    ...
    card.className = 'repo-card reveal in-view';
    card.innerHTML = `
...
form.addEventListener('submit', e => {
  ...
  if (!validEmail(formEmail.value.trim())) {
